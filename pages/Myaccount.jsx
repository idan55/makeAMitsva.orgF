    ...
    {user && (
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ marginTop: "20px" }}>
          <p><strong>Stars:</strong> {stars} / 500</p>
          <div
            style={{
              width: "100%",
              height: "20px",
    ...
    {!loading && !error && (
      ...
      >
          ...
          >
            {myOpenRequests.map((req) => (
              ...
              >
                ...
                <div>Posted: {new Date(req.createdAt).toLocaleString()}</div>
                <p>{req.description}</p>
                {req.completedBy ? (
                  <button onClick={() => handleSolved(req._id)}>Solved</button>
                ) : (
                  <p style={{ fontSize: "12px", color: "#666" }}>No helper yet.</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          {mySolvedForOthers.length === 0 && <p>You have not completed any requests yet.</p>}
          ...
          >
            {mySolvedForOthers.map((req) => (
              ...
              >
                <strong>{req.title}</strong>
                <div>
                  Help seeker: {req.createdBy?.name || req.createdBy?.email || "Unknown"}
                </div>
                <div>
                  Completed at: {new Date(req.updatedAt || req.createdAt).toLocaleString()}
                </div>
                <p>{req.description}</p>
              </div>
            ))}
